...
if not st.session_state.logado:
    ...
    aba_login, aba_cadastro = st.tabs(["🔒 Entrar no Sistema", "📝 Cadastrar Nova Empresa"])
    
    with aba_login:
        with st.form("form_login"):
            st.subheader("Login de Empresa")
    ...
    with aba_cadastro:
        with st.form("form_cadastro"):
            st.subheader("Registar Nova Empresa")
            ...
            btn_salvar_cadastro = st.form_submit_button("Registar Empresa")
            ...
            if btn_salvar_cadastro:
                if not novo_nome or not novo_login or not nova_senha:
                    ...
                    try:
                        ...
                        conn.commit()
                        conn.close()
                        st.success("Empresa cadastrada com sucesso! Agora vá para a aba 'Entrar no Sistema'.")
    ...
    # ==================== TELA: HISTÓRICO & EDICÃO ====================
        ...
        # O filtro 'WHERE empresa_id = ...' garante privacidade completa entre empresas parceiras
        conn = get_connection()
